...
if (!jeedom::apiAccess(init('apikey'), 'alexafiretv')) {
    ...
    log::add('alexafiretv_mqtt', 'debug', 'Clé Plugin Invalide');
    die();
}
...
$nom = $_GET["nom"];
log::add('alexafiretv', 'debug', 'Réception données sur jeealexafiretv [' . $nom . ']');
log::add('alexafiretv_mqtt', 'info', " -------------------------------------------------------------------------------------------------------------");
log::add('alexafiretv_widget', 'info', " -------------------------------------------------------------------------------------------------------------");

log::add('alexafiretv_mqtt', 'debug', "chaineRecuperee: " . $chaineRecuperee);
...
log::add('alexafiretv_mqtt', 'debug', "->" . $chaineRecupereeCorrigee);
...
//log::add('alexafiretv_mqtt', 'debug',  "chaineRecupereeCorrigee: ".$chaineRecupereeCorrigee);
//log::add('alexafiretv_mqtt', 'debug',  "nom: ".$nom);

$result = json_decode($chaineRecupereeCorrigee, true);

//log::add('alexafiretv_mqtt', 'debug',  "result: ".$result); // Attention déclenche un PHP Notice:  Array to string conversion

if (!is_array($result)) {
    log::add('alexafiretv_mqtt', 'debug', 'Format Invalide');
    die();

}
//log::add('alexafiretv_mqtt', 'debug',  'deviceSerialNumber:'.$result['deviceSerialNumber']);
$logical_id = $result['deviceSerialNumber'] . "_player";

//$alexafiretv_player=eqLogic::byLogicalId($logical_id, 'alexaamazonmusic'); // PLAYER
$alexafiretv2 = eqLogic::byLogicalId($result['deviceSerialNumber'], 'alexafiretv'); // ECHO
//$alexafiretv3=alexaamazonmusic::byLogicalId($result['deviceSerialNumber']."_playlist", 'alexaamazonmusic'); // PLAYLIST
...
//log::add('alexafiretv_node', 'info',  'Alexa-jee: '.$nom);

switch ($nom) {

    case 'commandesEnErreur':
        log::add('alexafiretv_node', 'warning', "Alexa-jee: Il va falloir relancer: " . $chaineRecupereeCorrigee . " Pause 8s");
        sleep(8);
    ...
    if ($result['bluetoothEvent'] == 'DEVICE_DISCONNECTED') metAJour("bluetoothDevice", "Déconnexion en cours", 'bluetoothDevice', false , "ECHO", $result['deviceSerialNumber']);
        metAJourBluetooth($result['deviceSerialNumber'], $result['audioPlayerState'], $alexafiretv2, "PLAYER", $result['deviceSerialNumber']);
    break;
    */
    ...
    case 'ws-notification-change': //changement d'une alarme/rappel
        log::add('alexafiretv_node', 'info', 'Alexa-jee: notificationVersion: ' . $result['notificationVersion']);

        $alexafiretv2->refresh();    // Lance un refresh du device principal
        break;
    ...
    case 'ws-device-activity':
        //log::add('alexafiretv_mqtt', 'debug',  '1');
        ...
        if ((isset($result['description']['summary'])) && ($result['description']['summary'] != "")) {
            ...
            $alexafiretveqlogic = eqLogic::byLogicalId($result['deviceSerialNumber'], 'alexafiretv');
            log::add('alexafiretv', 'debug', 'Interaction ' . $alexafiretveqlogic->getConfiguration('interactionjeedom') == 1);
            if ($alexafiretveqlogic->getConfiguration('interactionjeedom') == 1) {
                $demandeinteract = $result['description']['summary'];
                if (strpos($demandeinteract, 'jacques dit') === false && $demandeinteract != "alexa") {
                    log::add('alexafiretv', 'debug', 'Interaction demande : ' . $demandeinteract);

                    $parameters['plugin'] = 'alexafiretv';
                    $reply = interactQuery::tryToReply(trim($demandeinteract), $parameters);
                    log::add('alexafiretv', 'debug', 'Interaction ' . print_r($reply, true));
                    if ($reply['reply'] != "Désolé je n'ai pas compris" && $reply['reply'] != "Désolé je n'ai pas compris la demande" && $reply['reply'] != "Désolé je ne comprends pas la demande" && $reply['reply'] != "Je ne comprends pas" && $reply['reply'] != "ceci est un message de test" && $reply['reply'] != "" && $reply['reply'] != " ") {
                        log::add('alexafiretv', 'debug', "La reponse : " . $reply['reply'] . " est valide je vous l'ai donc renvoyée");
                        $cmd = $alexafiretveqlogic->getCmd('action', 'speak');
                        $option = array('message' => $reply['reply']);
                        $cmd->execute($option);
                    } else {
                        log::add('alexafiretv', 'debug', "La reponse : " . $reply['reply'] . " est une reponse générique je vous l'ai donc pas renvoyée");
                    }
                }
            }
        ...
        //log::add('alexafiretv_mqtt', 'debug',  '2');
        ...
        //metAJour("songName", $result['domainAttributes']['nBestList']['songName'], 'songName', true , $alexafiretv);

        break;
    case 'message_add':
        //log::add('alexafiretv_node', 'info',  'Alexa-jee: '.$result['message']);
        message::add('alexafiretv', $result['message']);
        break;
    ...
    case 'refreshPlayer':
        //log::add('alexafiretv_widget', 'debug', '*********************************rrrrefreshPlayer avec '.$result['playerInfo']['miniArt']['url']."//".json_encode($result).'********************');
    ...
    default:
        ...
        if (!is_object($alexafiretv2)) {
            log::add('alexafiretv_mqtt', 'debug', 'Device non trouvé: ' . $logical_id);
            die();
        } else {
            log::add('alexafiretv_mqtt', 'debug', 'Device trouvé: ' . $logical_id);
        }
...
//	if (is_object($alexafiretv)) $alexafiretv_player->refreshWidget();
...
function metAJour($nom, $variable, $commandejeedom, $effaceSiNull, $_typeDevice, $_deviceSerialNumber)
{
    ...
    if ($_typeDevice == "ECHO") {
        $alexafiretv2 = eqLogic::byLogicalId($_deviceSerialNumber, 'alexafiretv'); // ECHO
        if (is_object($alexafiretv2)) metAJour2($nom, $variable, $commandejeedom, $effaceSiNull, $alexafiretv2);
    }

    if ($_typeDevice == "PLAYER") {
        foreach (alexafiretv::listePluginsAlexa() as $pluginAlexaUnparUn) {
            $alexafiretv = eqLogic::byLogicalId($_deviceSerialNumber . "_player", $pluginAlexaUnparUn); // PLAYER
            if (is_object($alexafiretv)) metAJour2($nom, $variable, $commandejeedom, $effaceSiNull, $alexafiretv);
        }
    }

    if ($_typeDevice == "PLAYLIST") {
        foreach (alexafiretv::listePluginsAlexa() as $pluginAlexaUnparUn) {
            $alexafiretv3 = eqLogic::byLogicalId($_deviceSerialNumber . "_playlist", $pluginAlexaUnparUn); // PLAYLIST
            if (is_object($alexafiretv3)) metAJour2($nom, $variable, $commandejeedom, $effaceSiNull, $alexafiretv3);
        }
    }
}

function metAJour2($nom, $variable, $commandejeedom, $effaceSiNull, $_alexafiretv)
{
    try {
        if (isset($variable)) {
            if ($nom != 'playlisthtml') { // on supprime playlisthtml des logs sinon ils deviennent illisibles
                log::add('alexafiretv_widget', 'info', '   [' . $nom . ':' . $commandejeedom . '] find: ' . json_encode($variable) . " sur {" . $_alexafiretv->getName() . "}");
                log::add('alexafiretv_mqtt', 'info', '   [' . $nom . ':' . $commandejeedom . '] find: ' . json_encode($variable) . " sur {" . $_alexafiretv->getName() . "}");
            }
            $_alexafiretv->checkAndUpdateCmd($commandejeedom, $variable);
        } else {
            log::add('alexafiretv_widget', 'info', '   [' . $nom . ':' . $commandejeedom . '] non trouvé: ' . $variable);
            if ($effaceSiNull) {
                $_alexafiretv->checkAndUpdateCmd($commandejeedom, null);
                log::add('alexafiretv_widget', 'info', '   [' . $nom . ':' . $commandejeedom . '] non trouvé et vidé');
            }
        }
    } catch (Exception $e) {
        log::add('alexafiretv_mqtt', 'info', ' [' . $nom . ':' . $commandejeedom . '] erreur1: ' . $e);

    } catch (Error $e) {
        log::add('alexafiretv_mqtt', 'info', ' [' . $nom . ':' . $commandejeedom . '] erreur22: ' . $e);
...
{
//log::add('alexafiretv_mqtt', 'debug',  'metAJourImage >>>>>>>'.$_deviceSerialNumber);
    foreach (alexafiretv::listePluginsAlexa() as $pluginAlexaUnparUn) {
        $alexafiretv = eqLogic::byLogicalId($_deviceSerialNumber . "_player", $pluginAlexaUnparUn); // PLAYER
        metAJourImage2($nom, $variable, $commandejeedom, $effaceSiNull, $alexafiretv);
    }
}

function metAJourImage2($nom, $variable, $commandejeedom, $effaceSiNull, $_alexafiretv)
{
//log::add('alexafiretv_mqtt', 'debug',  'metAJourImage2 '.$nom."/". $variable."/".$commandejeedom);

    try {
        if (isset($variable)) {
            log::add('alexafiretv_widget', 'info', '   [' . $nom . ':' . $commandejeedom . '] find: ' . json_encode($variable));
            log::add('alexafiretv_mqtt', 'info', '   [' . $nom . ':' . $commandejeedom . '] find: ' . json_encode($variable));
            $_alexafiretv->checkAndUpdateCmd($commandejeedom, $variable);
        } else {
            log::add('alexafiretv_mqtt', 'debug', '[' . $nom . ':' . $commandejeedom . '] non trouvé');
            $_alexafiretv->checkAndUpdateCmd($commandejeedom, "plugins/alexafiretv/core/img/vide.gif");
        }
    } catch (Exception $e) {
        log::add('alexafiretv_mqtt', 'info', ' [' . $nom . ':' . $commandejeedom . '] erreur1: ' . $e);

    } catch (Error $e) {
        log::add('alexafiretv_mqtt', 'info', ' [' . $nom . ':' . $commandejeedom . '] erreur21: ' . $e);
    }
}
...
function metAJourStatusPlayer($_Status, $_deviceSerialNumber)
{
    foreach (alexafiretv::listePluginsAlexa() as $pluginAlexaUnparUn) {
        ...
        if (is_object($alexafiretv)) {
            $alexafiretv->setStatus('Playing', $_Status);
            $alexafiretv->refreshWidget(); //refresh Tuile Player
        }
    }
}
...
{
    log::add('alexafiretv_widget', 'info', " ***********************[metAJourPlayer]*********************************" . $serialdevice);

    try {
        $json = file_get_contents("http://" . config::byKey('internalAddr') . ":3456/playerInfo?device=" . $serialdevice);
        $result = json_decode($json, true);
        log::add('alexafiretv_widget', 'debug', ' JSON:' . $json);
        ...
        //$alexafiretv_player->setStatus('Playing', ($result['playerInfo']['state']=="PLAYING"));

    } catch (Exception $e) {
        log::add('alexafiretv_widget', 'info', ' [' . $serialdevice . '] erreur1: ' . $e);
    } catch (Error $e) {
        log::add('alexafiretv_widget', 'info', ' [' . $serialdevice . '] erreur25: ' . $e);
    }
//if (is_object($alexafiretv_player)) $alexafiretv_player->refreshWidget(); //refresh Tuile Player
    log::add('alexafiretv_widget', 'debug', '** Mise à jour Tuile du Player **');
}

function metAJourPlaylist($serialdevice, $audioPlayerState, $_quiMetaJour = 'personne')
{
    //log::add('alexafiretv_widget', 'debug', '*********************************metAJourPlaylist par '.$_quiMetaJour.'********************');
    //log::add('alexafiretv_widget', 'debug', '*********************************metAJourPlaylist AVEC '.$result.'********************');
    try {
//		if (($audioPlayerState!="FINISHED") && (isset($result['playerInfo']['miniArt']['url']))) 	{	//modif Sigalou 24/08/2020 ? Pourquoi 2eme partie du test ?
        if (($audioPlayerState != "FINISHED")) {
            ...
            //sleep(2);
            //log::add('alexafiretv_widget', 'debug', '*********************************1metAJourPlaylist par '.$_quiMetaJour.'********************');
            $json = file_get_contents("http://" . config::byKey('internalAddr') . ":3456/playerinfo?device=" . $serialdevice);
            $result = json_decode($json, true);
            ...
            //log::add('alexafiretv_widget', 'debug', '-----------------subText1:'.$result['playerInfo']['infoText']['subText1']);
            //log::add('alexafiretv_widget', 'debug', '-----------------subText2:'.$result['playerInfo']['infoText']['subText2']);
            //log::add('alexafiretv_widget', 'debug', '-----------------title:'.$result['playerInfo']['infoText']['title']);


            $json = file_get_contents("http://" . config::byKey('internalAddr') . ":3456/media?device=" . $serialdevice);
            $result = json_decode($json, true);
            //log::add('alexafiretv_widget', 'debug', '-----------------result:'.json_encode($result));
        } else {
            //log::add('alexafiretv_widget', 'debug', '*********************************1BISmetAJourPlaylist par '.$_quiMetaJour.'********************');
        }

        if (isset($result)) {
            //log::add('alexafiretv_widget', 'debug', '*********************************2metAJourPlaylist par '.$_quiMetaJour.'********************');
            ...
            if (isset($result['queue'])) {
                foreach ($result['queue'] as $key => $value) {
                    //log::add('alexafiretv_widget', 'debug', '>>>>>>>>>>>>>>>>>album:'.$value['album']."/".$album_enCoursdeLecture);
                    //log::add('alexafiretv_widget', 'debug', '>>>>>>>>>>>>>>>>>artist:'.$value['artist']."/".$artist_enCoursdeLecture);
                    //log::add('alexafiretv_widget', 'debug', '-----------------imageURL:'.$value['imageURL']);
                    //log::add('alexafiretv_widget', 'debug', '>>>>>>>>>>>>>>>>>title:'.$value['title']."/".$title_enCoursdeLecture);
                    //log::add('alexafiretv_widget', 'debug', '>>>>>>>>>>>>>>>>>album:'.$value['album']."/".$album_enCoursdeLecture);
                    //log::add('alexafiretv_widget', 'debug', '>>>>>>>>>>>>>>>>>artist:'.$value['artist']."/".$artist_enCoursdeLecture);
                    //log::add('alexafiretv_widget', 'debug', '-----------------durationSeconds:'.$value['durationSeconds']);
                    ...
                    if (($value['artist'] == $artist_enCoursdeLecture) && ($value['title'] == $title_enCoursdeLecture) && $etatPlayer == "PLAYING")
                        $html .= "<img style='position:absolute' src='plugins/alexafiretv/core/img/playing_petit.gif' />";
                    $html .= "
				<object data='" . $value['imageURL'] . "' style='height: 60px;width: 60px;border-radius: 30%;' type='image/png'>
				<img style='height: 60px;width: 60px;border-radius: 30%;'  src='plugins/alexafiretv/core/img/musique.png'/></object></a></td>
            ...
            $html = "<br>";
            //log::add('alexafiretv_widget', 'debug', '*********************************2BISmetAJourPlaylist par '.$_quiMetaJour.'********************');

        }
        metAJour("playlisthtml", $html, 'playlisthtml', true, "PLAYLIST", $serialdevice);
        foreach (alexafiretv::listePluginsAlexa() as $pluginAlexaUnparUn) {
            $alexafiretv3 = eqLogic::byLogicalId($serialdevice . "_playlist", $pluginAlexaUnparUn); // PLAYLIST
            $alexafiretv3->refreshWidget(); //refresh Tuile Playlist
        }
    } catch (Exception $e) {
        log::add('alexafiretv_widget', 'info', ' [' . $serialdevice . '] erreur1: ' . $e);
    } catch (Error $e) {
        log::add('alexafiretv_widget', 'info', ' [' . $serialdevice . '] erreur26: ' . $e);
    }

    //log::add('alexafiretv_widget', 'debug', '*********************************FINmetAJourPlaylist par '.$_quiMetaJour.'********************');
    //log::add('alexafiretv_widget', 'debug', '*********************************FINmetAJourPlaylist par '.$_quiMetaJour.'********************');
...
/*
// Faudra le tester !!!!!!!!!!!!!!
function metAJourBluetooth($serialdevice, $audioPlayerState, $alexafiretv2, $alexafiretv_player) {
		//log::add('alexafiretv_widget', 'debug',  'zzzzzzzzzzzzzzzzz metAJourPlayer:');
...
//log::add('alexafiretv_widget', 'debug', '-->--->--->--->--deviceSerialNumber:'.$result['bluetoothStates']['0']['deviceSerialNumber']);
		...
		//log::add('alexafiretv_widget', 'debug',  '------------->'.json_encode($result));

		//if (($audioPlayerState=="PLAYING") || ($audioPlayerState=="REFRESH") || ($audioPlayerState=="PAUSED"))	{

		foreach ($result['bluetoothStates'] as $key => $value) {
				//log::add('alexafiretv_widget', 'debug', '-------------------------------------------------------------------------------');
				//log::add('alexafiretv_widget', 'debug', '-----------------deviceType:'.$value['deviceType']);
				//log::add('alexafiretv_widget', 'debug', '-----------------friendlyName:'.$value['friendlyName']);
				//log::add('alexafiretv_widget', 'debug', '-----------------online:'.$value['online']);
				//log::add('alexafiretv_widget', 'debug', '-----------------pairedDeviceList:'.$value['pairedDeviceList']);
				if (is_array($value['pairedDeviceList'])) {
					foreach ($value['pairedDeviceList'] as $key2 => $value2) {
						if ($value['deviceSerialNumber'] == $serialdevice) {
						//log::add('alexafiretv_widget', 'debug', '-----------------$serialdevice:'.$serialdevice);
						//log::add('alexafiretv_widget', 'debug', '-----------------deviceSerialNumber:'.$value['deviceSerialNumber']);
						//log::add('alexafiretv_widget', 'debug', '********** friendlyName:'.$value2['friendlyName']);
						//log::add('alexafiretv_widget', 'debug', '********** connected:'.$value2['connected']);
			...
			log::add('alexafiretv_widget', 'info',  ' ['.$nom.':'.$commandejeedom.'] erreur1: '.$e);

	} catch (Error $e) {
			log::add('alexafiretv_widget', 'info',  ' ['.$nom.':'.$commandejeedom.'] erreur27: '.$e);
